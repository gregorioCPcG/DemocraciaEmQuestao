...
# #### grafico teste de media dep 1 sem voto]
...
# grafico teste de média dep 3 sem voto e com amostra toda ####
...
# um gráfico de barras extra p/ H1 ####
df -> df55
...
# Calcular as porcentagens para TRUE em relação ao PL e PT
...
# Filtrar apenas as linhas para TRUE
...
# Carregar a biblioteca ggplot2 para gráficos
library(ggplot2)

# Criar gráfico para Tolerância ao PL
D2 <- ggplot(toleranciadf1, aes(x = Feeling, y = toleraPL)) +
  ...
  PL,Partido de Bolsonaro",
       x = "Sentimento",
       y = "Porcentagem (%)") +
  ylim(0, 35) +  # Ajustar o limite do eixo y conforme necessário
  theme_minimal()
...
# Criar gráfico para Tolerância ao PT
...
# Criação do df222 excluindo casos onde feeling é "Gosta Muito PL"
df222 <- df[df$feeling != "Gosta Muito PL", ]

# Criação do df224 excluindo casos onde feeling é "Gosta Muito PT"
